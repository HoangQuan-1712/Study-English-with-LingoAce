...
function CorrectFeedback({ card, goal, onNext }: {
    card: Card; goal: 'quick' | 'master'; onNext: () => void
}) {
    ...
    useEffect(() => {
        ...
        if (goal === 'master') {
            ...
            timer = setTimeout(doAdvance, 2500)
        }
        return () => clearTimeout(timer)
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []) // QUAN TRỌNG: empty deps - chỉ chạy khi mount, không re-run
...
export default function LearnPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleAnswer = (userAnswer: string) => {
        ...
        if (correct) {
            ...
            setCorrectKey(k => k + 1) // force remount CorrectFeedback
            setFbMode('correct')
        } else {
            setStreak(0)
        ...
        <div className="learn-fullscreen">
            ...
            <div className="learn-content">
                ...
                {fbMode === 'correct' && correctCard && (
                    <CorrectFeedback key={correctKey} card={correctCard} goal={goal} onNext={advanceStep} />
                )}
